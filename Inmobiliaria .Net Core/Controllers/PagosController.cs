...
namespace Inmobiliaria_.Net_Core.Controllers
{
    public class PagosController : Controller
    {
        ...
        public ActionResult Index(int id)
        {
            ...
            {
                if (a.Importe < alquiler.MontoTotal) { deuda = deuda+ (alquiler.MontoTotal - a.Importe);  if (sobrante>0) {sobrante= sobrante-(alquiler.MontoTotal-a.Importe); } }
                    else if (a.Importe>=alquiler.MontoTotal) { sobrante = sobrante + (a.Importe - alquiler.MontoTotal) - deuda; deuda = deuda -(a.Importe-alquiler.MontoTotal); }
            }
            if (deuda <= 0) { deuda = 0; } else { sobrante = 0; }
        ...
        public ActionResult Delete(int id)
        {
            ...
            var dos = Alquileres.ObtenerPorId(uno.Alquiler.IdContrato);
            ViewBag.alquiler = dos;
            ViewBag.trato = uno.NroPago + " para el alquiler " + uno.Alquiler.IdContrato + " en el inmueble ubicado en " + dos.Inmueble.Direccion + " por $" + uno.Importe + "- para la fecha " + uno.Fecha;
            return View(uno);
        }
        [Authorize]
        [HttpPost]
